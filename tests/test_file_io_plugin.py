...
def test_file_io_plugin_unit(tmp_path: Path) -> None:
    ...
    result = plugin.run(
        state,
        {"operation": "update", "path": str(path), "content": "world"},
    )
    ...
    result = plugin.run(
        state,
        {"operation": "delete", "path": str(path), "confirm": True},
    )
    ...
    result = plugin.run(
        state,
        {"operation": "create", "path": str(outside), "content": "oops"},
    )
...
def test_file_io_plugin_path_sanitization(tmp_path: Path) -> None:
    ...
    result = plugin.run(
        state,
        {"operation": "create", "path": str(inside), "content": "hi"},
    )
    assert result == {"status": "created"}
    assert state["last_file_path"] == str(tmp_path / "sanitized.txt")

    # Path that escapes the allowed directory is rejected and does not
    # update state
    outside = tmp_path.parent / "other.txt"
