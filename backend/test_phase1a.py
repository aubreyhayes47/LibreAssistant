...
from utils import get_logger, init_config, init_logging  # noqa: E402
...
async def test_database_operations():
    ...
    response = await ChatOperations.save_message(
        "Hello there! How can I help you?", "assistant", "test_session"
    )
    ...
    print(
        f"Get chat history: {'SUCCESS' if len(history) == 2 else 'FAILED'} "
    ...
    print(
        f"Get bookmarks: {'SUCCESS' if len(bookmarks) == 1 else 'FAILED'} "
        f"({len(bookmarks)} bookmarks)"
    )

    search_results = await BookmarkOperations.search_bookmarks("example")
    print(
        f"Search bookmarks: {'SUCCESS' if len(search_results) == 1 else 'FAILED'} "
    ...
    history_success = await HistoryOperations.add_history_entry(
        "https://example.com", "Example Site", "test_session"
    )
    ...
    print(
        f"Get browser history: {'SUCCESS' if len(browser_history) == 1 else 'FAILED'} "
        f"({len(browser_history)} entries)"
    )
...
async def test_llm_integration():
    ...
    try:
        ...
        print(
            f"List models: {'SUCCESS' if isinstance(models, list) else 'FAILED'} "
            f"({len(models)} models)"
        )
        ...
        if models:
            ...
            print(
                f"Chat completion: {'SUCCESS' if result.get('success') else 'FAILED'}"
            )
...
async def test_command_handler():
    ...
    try:
        ...
        print(
            f"Init database command: {'SUCCESS' if result.get('success') else 'FAILED'}"
        )
        ...
        print(
            f"Save bookmark command: {'SUCCESS' if result.get('success') else 'FAILED'}"
        )

        # Test get bookmarks command
        result = await handler.handle_command("get_bookmarks", {"folder": "testing"})
        print(
            f"Get bookmarks command: {'SUCCESS' if result.get('success') else 'FAILED'}"
        )
        if result.get("success"):
