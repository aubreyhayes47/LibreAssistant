...
class ChatOperations:
    ...
    @staticmethod
    async def save_message(content: str, role: str, session_id: str = 'default') -> Optional[ChatMessage]:
    ...
    @staticmethod
    async def get_chat_history(session_id: str = 'default', limit: int = 50) -> List[Dict[str, Any]]:
...
class BookmarkOperations:
    ...
    @staticmethod
    async def save_bookmark(url: str, title: str = None, content: str = None, 
                          tags: str = None, folder: str = 'default') -> Optional[BookmarkEntry]:
        """Save a bookmark to the database."""
        try:
            with get_db_session() as db:
                # Check if bookmark already exists
                existing = db.query(BookmarkEntry).filter(BookmarkEntry.url == url).first()
                if existing:
    ...
    @staticmethod
    async def get_bookmarks(folder: str = None, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            with get_db_session() as db:
                ...
                bookmarks = query.order_by(desc(BookmarkEntry.timestamp))\
                    .limit(limit)\
                    .all()
                ...
                return [
                    {
                        ...
                        'url': bm.url,
                        'title': bm.title,
                        'content': bm.content[:500] if bm.content else None,  # Truncate content
                        'tags': bm.tags,
                        'folder': bm.folder,
    ...
    async def search_bookmarks(query: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            with get_db_session() as db:
                search_term = f"%{query}%"
                bookmarks = db.query(BookmarkEntry)\
                    .filter(or_(
    ...
    async def delete_bookmark(bookmark_id: int) -> bool:
        ...
        try:
            with get_db_session() as db:
                bookmark = db.query(BookmarkEntry).filter(BookmarkEntry.id == bookmark_id).first()
                if bookmark:
                    db.delete(bookmark)
                    return True
                return False
...
class HistoryOperations:
    ...
    @staticmethod
    async def add_history_entry(url: str, title: str = None, session_id: str = 'default') -> bool:
        """Add or update a history entry."""
        try:
            with get_db_session() as db:
                # Check if URL was visited recently (within last hour)
                recent_cutoff = datetime.utcnow() - timedelta(hours=1)
                existing = db.query(BrowserHistory)\
                    .filter(and_(
    ...
    @staticmethod
    async def get_history(session_id: str = None, limit: int = 100) -> List[Dict[str, Any]]:
        """Get browser history."""
        try:
            with get_db_session() as db:
                ...
                history = query.order_by(desc(BrowserHistory.visit_time))\
                    .limit(limit)\
                    .all()
    ...
    async def search_history(query: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            with get_db_session() as db:
                search_term = f"%{query}%"
                history = db.query(BrowserHistory)\
                    .filter(or_(
...
class SummaryOperations:
    ...
    @staticmethod
    async def save_summary(url: str, summary: str, content: str, model_used: str = None) -> Optional[PageSummary]:
        """Save a page summary."""
        try:
            with get_db_session() as db:
    ...
    @staticmethod
    async def get_summary(url: str, content_hash: str = None) -> Optional[Dict[str, Any]]:
