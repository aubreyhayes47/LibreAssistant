...
class WebScrapingConfig:
    ...
    def validate(self) -> bool:
        ...
        return (
            self.request_timeout > 0 and
            self.max_page_size > 0 and
...
class ConfigManager:
    ...
    def _apply_env_overrides(self, config: LibreAssistantConfig) -> LibreAssistantConfig:
        ...
        if os.getenv('LIBREASSISTANT_LLM_TEMPERATURE'):
            try:
                config.llm.temperature = float(os.getenv('LIBREASSISTANT_LLM_TEMPERATURE'))
            except ValueError:
                pass
        
        # Debug mode
        if os.getenv('LIBREASSISTANT_DEBUG'):
            config.debug_mode = os.getenv('LIBREASSISTANT_DEBUG').lower() in ('true', '1', 'yes')
