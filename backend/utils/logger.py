...
class LibreAssistantLogger:
    ...
    def _setup_loggers(self):
        ...
        self._create_logger("libreassistant.main", self.log_files["main"], logging.INFO)

        # Database operations logger
        self._create_logger(
            "libreassistant.database", self.log_files["database"], logging.INFO
        )

        # LLM operations logger
        self._create_logger("libreassistant.llm", self.log_files["llm"], logging.INFO)

        # Web scraping logger
        self._create_logger(
            "libreassistant.web", self.log_files["web_scraping"], logging.INFO
        )

        # Error logger (for all error-level messages)
        self._create_logger(
            "libreassistant.errors", self.log_files["errors"], logging.ERROR
        )
        ...
        file_handler = logging.handlers.RotatingFileHandler(
            self.log_files["main"], maxBytes=10 * 1024 * 1024, backupCount=5  # 10MB
        )
    ...
    def log_command_success(
        self, command: str, duration: float, session_id: str = None
    ):
        """Log successful command completion."""
        logger = self.get_logger("main")
        extra = {"command": command, "session_id": session_id, "duration": duration}
        logger.info(
            f"Command '{command}' completed successfully in {duration:.2f}s",
            extra=extra,
        )
    ...
    def log_database_operation(
        self, operation: str, table: str, success: bool, details: str = None
    ):
    ...
    def log_llm_interaction(
        self, model: str, prompt_length: int, response_length: int, duration: float
    ):
